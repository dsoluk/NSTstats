...
def _expected_per_game(row: pd.Series, stat: str, season_weight: float, last7_weight: float) -> float:
    """Compute expected stat per game using per-60 rates and TOI per game.
    Uses season vs last7 weighting. PPP uses PP TOI; others use all-situations TOI.
    """
    # choose TOI columns based on stat
    if stat == "PPP":
        toi_cols = ("szn_TOI/GP_pp", "l7_TOI/GP_pp")
    else:
        toi_cols = ("szn_TOI/GP_all", "l7_TOI/GP_all")

    rate_cols = (f"szn_{stat}", f"l7_{stat}")

    szn_rate = row.get(rate_cols[0], np.nan)
    l7_rate = row.get(rate_cols[1], np.nan)

    szn_toi_min = _parse_toi_value(row.get(toi_cols[0], np.nan))
    l7_toi_min = _parse_toi_value(row.get(toi_cols[1], np.nan))
    ...
    return _safe_weighted_mean([szn_pg, l7_pg], [season_weight, last7_weight])
...
def forecast(
    ...
    for _, r in sk.iterrows():
        ...
        for stat in SKATER_STATS:
            per_game[stat] = _expected_per_game(r, stat, season_weight, last7_weight)
