...
> Upstream reference
> For the canonical, organization‑wide guidelines used by Junie Pro, see `.junie/guidelines.md`. This project document only adds NSTstats‑specific preferences on top of the org guide.
...
## 15) NSTstats project‑specific preferences
- Respect existing docs (e.g., `docs\PROCESS.md`, `docs\FORECAST.md`) and CLI flows (`app.cli`).
- Prefer targeted searches for symbols (e.g., in `app\`, `application\`, `adapters\`).
- When changing forecasting or DQ behavior, update the corresponding docs under `docs\` only if user‑facing.
- Use Windows‑style paths and PowerShell‑friendly commands when demonstrating runs.
